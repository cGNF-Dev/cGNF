...
class DAGConditioner(Conditioner):
    def __init__(self, in_size, hidden, out_size, cond_in=0, soft_thresholding=False, h_thresh=0., gumble_T=0.5,
                 hot_encoding=False, l1=0., nb_epoch_update=1, A_prior=None,  Z_Sigma=None, hard=False):
...
#         print(hot_encoding)
        if A_prior is None:
            ...
            #tensor(-inf)
            torch.exp(torch.tensor(float('inf'))*-1)
            #tensor(0.)
            torch.log(torch.exp(torch.tensor(float('inf'))*-1))
            #tensor(-inf)
            self.A = nn.Parameter(torch.ones(in_size, in_size) * 1.5 + torch.randn((in_size, in_size)) * .02, requires_grad=False)
        else:
            self.A = nn.Parameter(A_prior, requires_grad=False)
        self.Z_Sigma = nn.Parameter(Z_Sigma, requires_grad=False)
        self.in_size = in_size
        self.exponent = self.in_size % 50
        self.s_thresh = soft_thresholding
        self.h_thresh = h_thresh
        self.stoch_gate = False#True
...
#         print(type(hidden))
    ...
    def post_process(self, zero_threshold=None):
        if zero_threshold is None:
            zero_threshold = .1
            G = nx.DiGraph((self.soft_thresholded_A().data.clone().abs() > zero_threshold).float().detach().cpu().numpy(), create_using=nx.DiGraph)
    ...
    def stochastic_gate(self, importance, hard=False):#False, True
        if self.gumble:
            ...
            if hard:
                ...
                dim = p.shape[1]
                p = p.view(bs,-1)
                y = torch.cat((p.unsqueeze(-1),1-p.unsqueeze(-1)),dim=-1)
                shape = y.size()
                _, ind = y.max(dim=-1)
...
#             print(f'{e.shape}, {hot_encoding.shape}')
            e = torch.cat((e, hot_encoding), 1).view(x.shape[0], self.in_size, -1)
            full_e = self.embedding_net(e)
            # TODO Add context
            return full_e
    ...
    def depth(self):
        G = nx.DiGraph((self.A.detach() > 0).float().cpu().numpy(), create_using=nx.DiGraph)
